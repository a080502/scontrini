...
$scontrini = $db->fetchAll("
    ...
    WHERE $where_clause 
    ORDER BY nome_display ASC, s.data DESC, s.created_at DESC
", $params);

// Raggruppa scontrini per nome persona
$scontrini_raggruppati = [];
foreach ($scontrini as $scontrino) {
    $nome_display = $scontrino['nome_display'];
    if (!isset($scontrini_raggruppati[$nome_display])) {
        $scontrini_raggruppati[$nome_display] = [];
    }
    $scontrini_raggruppati[$nome_display][] = $scontrino;
}
...
$anni_where = array_filter($anni_where, function($condition) {
    return strpos($condition, 'YEAR(') === false && strpos($condition, 'MONTH(') === false;
});
...
<div class="scontrini-raggruppati">
    <?php foreach ($scontrini_raggruppati as $nome_display => $scontrini_gruppo): ?>
        <?php 
        ...
        <div class="gruppo-nome">
            <h3 class="nome-gruppo">
                <?php echo htmlspecialchars($nome_display); ?>
            ...
            <div class="totali-gruppo">
                <div class="totali-riga">
                    <strong>Totali per <?php echo htmlspecialchars($nome_display); ?>:</strong>
