...
try {
    ...
    if (empty($query)) {
        ...
        $nomi = $db->fetchAll("
            SELECT 
                COALESCE(nome_persona, numero) as nome,
                COUNT(*) as count_usage,
                MAX(data) as last_used
            FROM scontrini 
            WHERE {$where_clause} AND COALESCE(nome_persona, numero) IS NOT NULL
            GROUP BY COALESCE(nome_persona, numero)
        ...
        $where_conditions[] = "COALESCE(nome_persona, numero) LIKE ?";
        $params[] = "%{$query}%";
